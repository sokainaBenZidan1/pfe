...
@Component({
  selector: 'app-operation-stock-detail-edit-admin',
  templateUrl: './operation-stock-detail-edit-admin.component.html'
})
export class OperationStockDetailEditAdminComponent extends AbstractEditController<OperationStockDetailDto, OperationStockDetailCriteria, OperationStockDetailAdminService>   implements OnInit {
   ...
   public async openCreateOperationStock(operationStock: string) {
        const isPermistted = await this.roleService.isPermitted('OperationStock', 'edit');
        if (isPermistted) {
             this.operationStock = new OperationStockDto();
