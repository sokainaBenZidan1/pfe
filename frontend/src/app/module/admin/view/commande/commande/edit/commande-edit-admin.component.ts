...
export class CommandeEditAdminComponent extends AbstractEditController<CommandeDto, CommandeCriteria, CommandeAdminService>   implements OnInit {
   ...
   public addCommandeDetails() {
       ...
       if (this.errorMessages.length === 0) {
            if(this.commandeDetailsElement.id == null){
                this.item.commandeDetails.push(this.commandeDetailsElement);
            }else{
                const index = this.item.commandeDetails.findIndex(e => e.id == this.commandeDetailsElement.id);
                this.item.commandeDetails[index] = this.commandeDetailsElement;
            }
